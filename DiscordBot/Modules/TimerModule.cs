...
namespace DiscordBot.Modules
{
    ...
    public class TimerModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        [SlashCommand("sveglia", "Imposta una sveglia [Mezza notte di default]")]
        public async Task SetAlarm([Summary("notifica", "Cosa vuoi che ti ricordi?")] string text, [Summary("ora", "A che ora? [Default 0]"), MaxValue(23)] int hours = 0, [Summary("minuto", "A che minuto? [Default 0]"), MaxValue(59)] int minutes = 0, [Summary("minuti", "A che secondo? [Default 5]"), MaxValue(59)] int seconds = 5, [Summary("loop", "Vuoi il timer in loop?")] EAnswer loop = EAnswer.No, [Summary("privato", "Vuoi che lo mandi in privato?")] EAnswer inPrivate = EAnswer.No)
        {
            var date = new DateTime(DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, hours, minutes, seconds);
            var timer = new Utility.DiscordUserTimer(Guild: Context.Guild, User: Context.User, TextChannel: inPrivate == EAnswer.Si ? null : Context.Channel, Name: $"{Context.User.Id}:{DateTime.UnixEpoch.Second}", Text: text, TargetTime: date, Callback: TimerFinished, Loop: loop == EAnswer.Si);
            var embed = DiscordData.CreateEmbed("Timer impostato!", Description: $"Per {timer.NextTargetTime:dddd dd MMMM alle HH:mm:ss}");
            await RespondAsync(embed: embed);
        }
        ...
        private static async void TimerFinished(object? sender, System.Timers.ElapsedEventArgs args)
        {
            ...
            try
            {
                ...
                var embed = DiscordData.CreateEmbed("Timer trascorso", Description: $"Notifica: {timer.Text ?? "N/A"}", User: user);
