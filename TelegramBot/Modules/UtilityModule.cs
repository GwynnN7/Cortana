...
internal struct AnswerCommand(EAnswerCommands cmd, Message interactionMessage, string? cmdVal = null)
{
    ...
    public static async void ButtonCallback(ITelegramBotClient cortana, Update update, string command)
        {
            ...
            if (AnswerCommands.ContainsKey(chatId) && command != "leave")
            {
            ...
            {
                case "qrcode":
                    AnswerCommands.Add(chatId, new AnswerCommand(EAnswerCommands.Qrcode, update.CallbackQuery.Message));
                    await cortana.EditMessageText(chatId, messageId, "Write the content of the Qrcode");
                    break;
                case "notify":
                    AnswerCommands.Add(chatId, new AnswerCommand(EAnswerCommands.Notification, update.CallbackQuery.Message));
                    await cortana.EditMessageText(chatId, messageId, "Write the content of the message");
                    break;
                case "join":
                    if (update.CallbackQuery.Message.Chat.Type == ChatType.Private)
                    {
                        AnswerCommands.Add(chatId, new AnswerCommand(EAnswerCommands.Chat, update.CallbackQuery.Message));
                        await cortana.EditMessageText(chatId, messageId, "Write the user you want to start the chat with", replyMarkup: CreateLeaveButton());
                    }
                    else await cortana.AnswerCallbackQuery(update.CallbackQuery.Id, "You can only use this command in private chat", true);
                    break;
                case "leave":
                    if (AnswerCommands.TryGetValue(chatId, out AnswerCommand cmd) && cmd.Command == EAnswerCommands.Chat)
                    {
    ...
    public static async void HandleCallback(MessageStats messageStats, ITelegramBotClient cortana)
    {
        switch (AnswerCommands[messageStats.ChatId].Command)
        {
            case EAnswerCommands.Qrcode:
                ...
                CreateUtilityMenu(cortana, messageStats.Message);
                break;
            ...
            case EAnswerCommands.Chat:
                ...
                if (messageStats.TextList.Count == 1)
                {
                    try
                    {
                        AnswerCommands[messageStats.ChatId] = new AnswerCommand(EAnswerCommands.Chat, AnswerCommands[messageStats.ChatId].InteractionMessage, TelegramUtils.NameToId(messageStats.TextList[0]).ToString());
