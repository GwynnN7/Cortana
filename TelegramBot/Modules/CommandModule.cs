...
internal abstract class CommandModule : IModuleInterface
{
	...
	public static async Task HandleCallbackQuery(ITelegramBotClient cortana, CallbackQuery callbackQuery, string command)
	{
		...
		{
			case "reboot":
				string reboot = HardwareApi.Devices.CommandComputer(EComputerCommand.Reboot);
				await cortana.AnswerCallbackQuery(callbackQuery.Id, reboot);
				break;
			case "suspend":
				string suspend = HardwareApi.Devices.CommandComputer(EComputerCommand.Suspend);
				await cortana.AnswerCallbackQuery(callbackQuery.Id, suspend);
				break;
			case "swap_os":
			...
			case "sleep":
				...
				await cortana.AnswerCallbackQuery(callbackQuery.Id, "Entering sleep mode");
				break;
			case "cancel":
	...
	public static InlineKeyboardMarkup CreateButtons()
	{
		return new InlineKeyboardMarkup()
			...
			.AddNewRow()
			.AddButton("Swap OS", "command-swap_os")
